...
$$
, where $c(n_k) = (\frac{n_k}{N.max})^{\lambda}$ is the cutoff parameter, $\lambda$ is a tuning parameter tuned to achieve desired operating characteristics, and $Y_k1$ and $Y_k0$ are observed data from both arms. This monitoring rule specifies that, if there is a low probability that the win ratio is greater than 1.5, we will stop the trial early. The interim look time is inferred by the recruitment interval,  which is the time between enrolling two consecutive patients. For example, if we enroll 4 patients over 1 month, the recruitment interval is 0.25.
